...
parser.add_argument("--num_labels", type=int, default=1, help="task name")  #stsb,mrpc,qqp,cola, sst2, qnli,rte, mnli
...
def make_model_init(task: str, num_labels: int):
    def _init():
        if task == "stsb":
            return BertForSentenceClassification(config=model_config,
                                          bert_pretrained_model_dir=True)
    return _init
