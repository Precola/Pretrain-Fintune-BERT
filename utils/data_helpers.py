...
def pad_sequence(sequences, batch_first=False, max_len=None, padding_value=0):
    ...
    For example:
        ...
        # Output: torch.Size([25, 3])

    Arguments:
        sequences:
            batch_first: Whether to place the batch_size in the first dimension.
        padding_value: The value used for padding.
        max_len:
            - When max_len = 50, the padding will be applied to make each sample a fixed length of 50, truncating excess elements.
            - When max_len = None, padding will be applied based on the longest sample in the current batch.
...
def pad_sequence_ex(sequences, batch_first=False, max_len=None, padding_value=0, pad_to_multiple_of=None):
    """
    Pad a list of variable-length Tensors with `padding_value`.
    Supports fixed `max_len` or automatic padding to a multiple of N.
    """

    if max_len is None:
        # Automatically determine max length from the longest sequence
        max_len = max([s.size(0) for s in sequences])
    ...
    for tensor in sequences:
        if tensor.size(0) < max_len:
            # Pad the tensor to the target length with `padding_value`
            tensor = torch.cat([tensor, torch.tensor([padding_value] * (max_len - tensor.size(0)))], dim=0)
        else:
            # Truncate the tensor if it's longer than `max_len`
            tensor = tensor[:max_len]
        out_tensors.append(tensor)

    out_tensors = torch.stack(out_tensors, dim=1)
    if batch_first:
        return out_tensors.transpose(0, 1)  # Convert to (batch, seq_len) format
    return out_tensors  # Return in (seq_len, batch) format
...
def process_cache(unique_key=None):
    """
    Data preprocessing result cache decorator.
    :param : unique_key
    :return:
    """
    if unique_key is None:
        raise ValueError(
            "`unique_key` cannot be empty. "
            "Please specify relevant member variables of the dataset construction class, "
            "such as `['top_k', 'cut_words', 'max_sen_len']`.")

    def decorating_function(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            logging.info(f" ## The parameters for indexing the preprocessed cache file are:{unique_key}")
            obj = args[0]  # To get the class object, you can access it through the self parameter, which is the first parameter in the data_process(self, file_path=None) method.
            ...
            for k in unique_key:
                paras += f"{k}{obj.__dict__[k]}_"  # To iterate over all the parameters in an object
            cache_path = os.path.join(file_dir, paras[:-1] + '.pt')
            start_time = time.time()
            if not os.path.exists(cache_path):
                logging.info(f"The cache file at {cache_path} does not exist. Reprocessing and caching the data!")
                data = func(*args, **kwargs)
                with open(cache_path, 'wb') as f:
                    torch.save(data, f)
            else:
                logging.info(f"The cache file at {cache_path} exists. Loading the cache file directly!")
                with open(cache_path, 'rb') as f:
                    data = torch.load(f, weights_only=True)
            end_time = time.time()
            logging.info(f"The total time spent on data preprocessing is:{(end_time - start_time):.3f}s")
            return data
...
class LoadSingleSentenceClassificationDataset:
    ...
    def data_process(self, file_path=None):
        """
        # Convert each word in every sentence into an index based on a dictionary,
        # and also return the length of the longest sample in all the data.
...
class LoadSQuADQuestionAnsweringDataset(LoadSingleSentenceClassificationDataset):
    ...
    def data_process(self, file_path, is_training=False):
        ...
        """
        logging.info(f"## Use a sliding window. doc_stride = {self.doc_stride}")
        examples = self.preprocessing(file_path, is_training)
        all_data = []
        example_id, feature_id = 0, 1000000000
        # Since a sliding window is used,
        # one example may produce multiple training samples (referred to as features here).
        # Therefore, it is necessary to number them separately,
        # and this is mainly used in the post-processing of the results after prediction,
        # not during training.
        # Of course, we can just use `feature_id` here,
        # as each example essentially corresponds to a single question,
        # so the question ID and example ID are essentially the same.

        for example in tqdm(examples, ncols=80, desc="Traversing each question (sample)."):
            ...
            logging.debug(f"<<<<<<<<  Entering a new example.  >>>>>>>>>")
            logging.debug(f"## Processing the data. {__name__} is_training = {is_training}")
            logging.debug(f"## Question. id: {example[0]}")
            logging.debug(f"## Original question. text: {example[1]}")
            logging.debug(f"## Original description. text: {example[3]}")
            ...
            logging.debug(f"## The context length is: {context_ids_len}, and the remaining length (rest_len) is: {rest_len}.")
            if context_ids_len > rest_len:  # The length exceeds `max_sen_len`, sliding window is required.
                logging.debug(f"## Entering the sliding window. …… ")
                ...
                while True:
                    ...
                    logging.debug(f"## Sliding window range: {s_idx, e_idx},example_id: {example_id}, feature_id: {feature_id}")
                    ...
                    if is_training:
                        ...
                        if start_position >= s_idx and end_position <= e_idx:  # in train
                            logging.debug(f"## The answer exists within the sliding window.  -----> ")
                            ...
                            logging.debug(f"## Original answer: {answer_text} <===> Processed answer:"
    ...
    def generate_batch(self, data_batch):
        ...
        for item in data_batch:
            # Item: [Original sample ID, training feature ID,
            # input_ids, seg, start, end, answer text, question ID, input_tokens, ori_map]
            batch_example_id.append(item[0])  # Original sample ID.
            batch_feature_id.append(item[1])  # Training feature ID.
            batch_input.append(item[2])  # input_ids
            batch_seg.append(item[3])  # seg
            batch_label.append([item[4], item[5]])  # Start, end.
    ...
    def load_train_val_test_data(self, train_file_path=None,
        ...
        logging.info(f"## Successfully returned {len(train_iter.dataset)} training samples, "
                     f"{len(val_iter.dataset)} validation samples, "
                     f"and {len(test_iter.dataset)} test samples.")
    ...
    def write_prediction(self, test_iter, all_examples, logits_data, output_dir):
        ...
        # Here, we are combining all the predicted results for each qid, because each qid corresponds to a context
        # that, when processed with a sliding window, may generate multiple training samples. Each training sample
        # will correspond to a predicted logits.
        # Also, since we take the top n best logits, combining them will result in multiple predicted answers for one question.

        for k, v in prelim_predictions.items():
            # For each qid, sort all the predicted answers based on the sum of start_logit
            # and end_logit.
